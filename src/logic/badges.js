...
//  - 4 niveaux selon le temps écoulé depuis la dernière action :
//      < 24h        -> neutre   (rien à signaler)
//      24h – 3j      -> jaune
//      3j – 7j       -> orange
//      >= 7j         -> rouge
//  - une fois au rouge, le signal "revient" tous les 2 jours pour relancer
...
export function badgeStatus(chapitre, side, now = Date.now()) {
  ...
  let level
  if (elapsedDays < 1) level = BADGE_LEVELS.NEUTRE
  else if (elapsedDays < 3) level = BADGE_LEVELS.JAUNE
  else if (elapsedDays < 7) level = BADGE_LEVELS.ORANGE
  else level = BADGE_LEVELS.ROUGE

  // Une fois au rouge, on relance le signal (pulse visuel) tous les 2 jours
  // plutôt que de laisser le badge silencieux entre deux clics.
  const pulse = level === BADGE_LEVELS.ROUGE && Math.floor(elapsedDays - 7) % 2 === 0
